...
public class ServerEntryPoint : IServerEntryPoint, IDisposable
{
	...
	public ServerEntryPoint(ILibraryManager libraryManager, IProviderManager providerManager, ILogManager logManager, ISecurityManager securityManager, ILibraryMonitor libraryMonitor, ICollectionManager collectionManager, IApplicationPaths applicationPaths)
	{
	...
	public void Run()
	{
		...
		{
			Plugin.Instance.CreateAllCollections(new Progress<double>(), CancellationToken.None).ConfigureAwait(continueOnCapturedContext: false);
		}
	}
	...
	public async void OnConfigurationUpdated(PluginConfiguration oldConfig, PluginConfiguration newConfig)
	{
		...
		{
			await Plugin.Instance.CreateAllCollections(new Progress<double>(), CancellationToken.None).ConfigureAwait(continueOnCapturedContext: false);
		}
	}
